...
#   * the angle between rhat and the disk's projected kinematic major axis is constant at
#     21.034 deg, i.e. the SKY-PROJECTED geometry (inclination, sky azimuthal angle, which
...
#     f1   = e1  (in-plane PCA major axis of the inner young stars);   f2 = sign * e2,
#            which makes (f1, f2, n_L) right-handed, so phi INCREASES in the direction the
#            disk rotates.  (e1 x e2 = n_disk is verified numerically.)
...
# angular-momentum sign calibration and the velocity sign convention all at once.
#
# INTERPRETATION CAVEAT, stated on the figures: phi = 0 is the inner young-star PCA major
# axis, which for a near-axisymmetric disk is a weak reference direction.  A signal in the
# GALAXY-STACKED f(phi) therefore only appears if HVC incidence is tied to the inner-disk
# elongation.  WITHIN a galaxy the reference is irrelevant, so the per-galaxy modulation
# amplitude (fig18d) is the reference-free statistic.
# Neighbouring alphas are 0.45 kpc apart at rho = 25.6 kpc and are therefore NOT independent;
# Wilson intervals, which assume independence, are lower bounds on the true uncertainty.  The
# 15 deg bins (6.7 kpc of arc) and the galaxy-to-galaxy scatter band are the honest errors.
# ═══════════════════════════════════════════════════════════════════════════════════════
...
def check_geometry(SL, verbose=True):
    ...
    ok = SL.v_ism_v3b.notna()
    out["r_pred"] = float(np.corrcoef(pred[ok], SL.v_ism_v3b[ok])[0, 1])
    out["ratio"] = float(np.nanmedian(SL.v_ism_v3b[ok] / pred[ok]))
    if verbose:
        ...
        print(f"  |phi - phi_major| takes only the values {out['phi_major']} deg "
              f"-> sky projection fixed, only disk material rotates")
        print(f"  v_ISM(v3b) vs v_c sin(i) cos(phi-phi_major): r = {out['r_pred']:.3f}, "
...
def within_galaxy_excess(K, N):
    """Per galaxy: observed RMS of f(phi), binomial shot-noise RMS, excess, chi2/dof."""
    fg = frac(K.sum(1), N.sum(1))
    f = frac(K, N)
    var_obs = np.nanvar(f, axis=1)
    var_shot = np.nanmean(fg[:, None] * (1 - fg[:, None]) / np.maximum(N, 1), axis=1)
    chi2 = np.nansum((K - N * fg[:, None]) ** 2 /
                     np.maximum(N * fg[:, None] * (1 - fg[:, None]), 1e-9), axis=1)
    return (np.sqrt(var_obs), np.sqrt(var_shot),
            np.sqrt(np.clip(var_obs - var_shot, 0, None)), chi2 / (NBIN - 1))
...
def fig18(D):
    ...
    # ── (a) HVC detection fraction vs azimuth, per line ────────────────────────────────
    a = ax[0, 0]
    for L in V.LINES:
    ...
    # galaxy-to-galaxy scatter for the primary line
    fp = frac(rep[PRIMARY]["K"], rep[PRIMARY]["N"])
    pc = V.LINE_BY_KEY[PRIMARY].color
    for q in (16, 84):
        a.plot(xc, np.nanpercentile(fp, q, axis=0), color=pc, ls=(0, (4, 2)), lw=1.1,
               alpha=0.85, label=(r"galaxy-to-galaxy $16/84$th pct "
    ...
    a.set_ylabel(r"HVC detection fraction $f_{\rm c}(|\Delta v|\geq100)$")
    ...
    # ── (c) blue- vs red-shifted HVC detection fraction vs azimuth ────────────────────
    c = ax[1, 0]
    asym = {}
    for L in [V.LINE_BY_KEY[k] for k in REP3]:
        for sgn, col in (("blue", S.INFLOW), ("red", S.OUTFLOW)):
            ...
            f = frac(K.sum(0), N.sum(0))
            c.plot(xc, f, color=col, ls=L.ls, lw=L.lw, alpha=0.95)
            if L.key == PRIMARY:
                lo, hi = V.wilson(K.sum(0), N.sum(0))
                c.fill_between(xc, lo, hi, color=col, alpha=0.20, lw=0)
            asym.setdefault(L.key, {})[sgn] = (K, N, f)
    hl = [plt.Line2D([], [], color="0.25", ls=V.LINE_BY_KEY[k].ls,
                     lw=V.LINE_BY_KEY[k].lw, label=V.LINE_BY_KEY[k].label) for k in REP3]
    ...
    c.set_ylim(0, 1.55 * np.nanmax([v[2] for d_ in asym.values() for v in d_.values()]))
    c.set_xlabel(r"disk azimuth of the impact point $\phi$ [deg]")
    c.set_ylabel(r"signed HVC detection fraction")
    ...
    for k in REP3:
        ...
        txt.append(r"%s: $f_{\rm blue}-f_{\rm red}=%+.3f$"
                   % (V.ION_LAB[V.LINE_BY_KEY[k].ion], (kb - kr) / nn))
    ...
    wg, sh, ex, chi2 = [], [], [], []
    for L in V.LINES:
        o, s_, e_, c_ = within_galaxy_excess(rep[L.key]["K"], rep[L.key]["N"])
        wg.append(np.nanmedian(o)); sh.append(np.nanmedian(s_))
        ex.append(np.nanmedian(e_)); chi2.append(np.nanmedian(c_))
        rep[L.key].update(wg_rms=np.nanmedian(o), shot=np.nanmedian(s_),
                          excess=np.nanmedian(e_), chi2=np.nanmedian(c_))
    ...
    lb = lb[:2] + [r"null (shift 95th pct $|$ binomial)"]
    d.set_xticks(xk)
    d.set_xticklabels([L.label for L in V.LINES], rotation=42, ha="right", fontsize=7.4)
    d.set_ylabel(r"azimuthal modulation amplitude of $f_{\rm c}$")
    d.set_ylim(0, 1.30 * max(np.nanmax(wg), np.nanmax(st_rms)))
    leg_ul(d, handles=hd, labels=lb, fontsize=7.6, framealpha=0.9)
    S.grid(d); S.panel_label(d, "(d)")
    S.tag(d, r"bar above its null $\Rightarrow$ real azimuthal structure", "ur", fs=7.6)

    fig.tight_layout()
    D["fig18"] = rep
    return S.save(fig, "fig18_covfrac_vs_orientation")
...
def fig19(D):
    ...
    a.set_ylim(-0.8, len(order) - 0.2); a.set_xlim(0, 0.86)
    a.set_xlabel(r"HVC detection fraction $f_{\rm c}$")
    ...
    def prop_panel(ax_, xv, xlab, lines, logx=False):
        ...
        lab = "\n".join([r"%s: $r_{\rm s}=%+.2f$, $p=%.3f$" %
                         (V.ION_LAB[V.LINE_BY_KEY[k].ion], r_, p_) for k, r_, p_ in txt])
        tag_at(ax_, lab, 0.115, 0.985, fs=7.2)
        if logx:
            ax_.set_xscale("log")
        ax_.set_xlabel(xlab)
        ax_.set_ylabel(r"HVC detection fraction $f_{\rm c}$")
    ...
    e.set_ylabel(r"$f_{\rm blue}-f_{\rm red}$  (HVC)")
    yl = e.get_ylim()
...
def fig20(D):
    ...
    for j, (nm, s_) in enumerate(mx.items()):
        ...
        a.plot(xk + (j - 0.5) * 0.38, fm, "D", ms=5.6, mfc="white", mec="0.1", mew=1.4,
               ls="none", zorder=6,
               label=(r"galaxy-matched ($n_{\rm gal}=%d$)" % len(mixed)) if j == 0 else None)
    ...
    a.set_ylabel(r"HVC detection fraction $f_{\rm c}$"); a.set_ylim(0, 0.86)
    leg_ul(a, fontsize=7.4, ncol=1, framealpha=0.92)
    S.grid(a); S.panel_label(a, "(a)")
    nfb = int(SLf.fb.sum())
    S.tag(a, (r"$%d/%d$ sightlines use the fallback" % (nfb, len(SLf))) + "\n"
          + r"numbers: paired $\Delta f_{\rm c}$ (matched galaxies)" + "\n" + D["stag"],
          "ur", fs=7.0)
    ...
    b.set_ylabel(r"signed HVC detection fraction")
    ...
    fig.tight_layout()
    D["fig20"] = res
    return S.save(fig, "fig20_edge_fallback_systematics")
...
def fig21(D):
    ...
    a.set_xticks(xk)
    a.set_xticklabels([L.label for L in V.LINES], rotation=42, ha="right", fontsize=7.4)
    a.set_ylabel(r"HVC detection fraction $f_{\rm c}$"); a.set_ylim(0, 0.66)
    ...
    b.set_xticks(xk)
    b.set_xticklabels([L.label for L in V.LINES], rotation=42, ha="right", fontsize=7.4)
    b.set_ylabel(r"HVC detection fraction $f_{\rm c}$"); b.set_ylim(0, 0.66)
    ...
    fig.tight_layout()
    D["fig21"] = res
    return S.save(fig, "fig21_vism_variant_sensitivity")
...
def report(D):
    ...
    print("\n[sample] HVC detection fraction, RAW vs CLEAN "
    ...
    r18 = D["fig18"]
    print("\n[fig18] azimuthal modulation of the HVC detection fraction "
          f"({NBIN} bins of {BW:.0f} deg)")
    print(f"{'line':14s} {'<f_c>':>7s} {'stack RMS':>10s} {'null95':>8s} {'p':>7s} "
          f"{'gal RMS':>8s} {'shot':>7s} {'excess':>7s} {'chi2/dof':>9s}")
    for L in V.LINES:
        r = r18[L.key]
    ...
    print("               circularly shifted (preserves each galaxy's own structure and the")
    print("               alpha-to-alpha correlation; destroys only the inter-galaxy alignment)")
    print("  gal RMS / shot / excess = median over galaxies of the within-galaxy RMS of")
    print("               f(phi), its binomial expectation, and the excess in quadrature")
    ...
    print("\n[fig19] per-galaxy HVC detection fraction")
    per = D["fig19"]["per"]
    for L in [V.LINE_BY_KEY[k] for k in REP]:
        t = per[L.key]
    ...
    for key, xlab, rs, ps in D["spearman"]:
        star = "  *" if ps < 0.05 else ""
        print(f"  {key:14s} vs {xlab:34s} r_s = {rs:+.3f}  p = {ps:.4f}{star}")
