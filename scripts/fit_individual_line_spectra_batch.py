#!/usr/bin/env python3
"""Production CLI for fitting per-line saved synthetic spectra with pygad.

This is a copied variant of fit_saved_spectra_batch.py.  The original fitter
loads the combined LSF spectrum and fits each transition in the blended full
spectrum.  This variant instead loads:

    /spectrum_by_line/<line_label_sanitized>/<branch>/{lambda_A, flux}

for each fitted line, using branch="lsf" by default.
"""
...
def line_group_name(saved_line: str) -> str:
    return (
        str(saved_line)
        .strip()
        .replace(" ", "_")
...
def selected_line_labels(args: argparse.Namespace) -> List[str]:
    raw = str(args.line_labels).strip()
    if raw.lower() in {"", "all", "*"}:
        return list(LINE_MAP.keys())
    requested = [item.strip() for item in raw.split(",") if item.strip()]
    bad = [item for item in requested if item not in LINE_MAP]
    if bad:
        raise ValueError(f"Unknown line label(s): {bad}. Valid labels: {list(LINE_MAP.keys())}")
    return requested


def load_individual_line_spectrum_h5(
    spectrum_file: str,
    saved_line: str,
    branch: str = "lsf",
) -> Tuple[np.ndarray, np.ndarray, Dict[str, Any]]:
    """Load one saved per-line spectrum branch from the combined ray HDF5."""
    group_name = line_group_name(saved_line)
    branch = str(branch).strip().lower()
    if branch not in {"lsf", "raw"}:
        raise ValueError(f"branch must be 'lsf' or 'raw'; got {branch!r}")
    base = f"spectrum_by_line/{group_name}/{branch}"
    with h5py.File(spectrum_file, "r") as h5:
        missing = [key for key in (f"{base}/lambda_A", f"{base}/flux") if key not in h5]
        if missing:
            raise KeyError(
                f"Missing per-line spectrum dataset(s) for {saved_line} in {spectrum_file}: {missing}"
            )
        wave = np.asarray(h5[f"{base}/lambda_A"], dtype=float)
        flux = np.asarray(h5[f"{base}/flux"], dtype=float)
        tau = np.asarray(h5[f"{base}/tau"], dtype=float) if f"{base}/tau" in h5 else None

    if wave.ndim != 1 or flux.ndim != 1 or len(wave) != len(flux):
        raise ValueError(
            f"Bad per-line spectrum shape for {saved_line}: wave={wave.shape}, flux={flux.shape}"
        )
    order = np.argsort(wave)
    meta = {
        "wave_key": f"{base}/lambda_A",
        "flux_key": f"{base}/flux",
        "tau_key": f"{base}/tau" if tau is not None else "",
        "line_group": group_name,
        "spectrum_branch": branch,
        "individual_line_spectrum_used": True,
    }
    return wave[order], flux[order], meta
...
def empty_line_error_record(
    ...
    return {
        ...
        "IndividualLineSpectrumUsed": True,
        "SpectrumBranch": "",
        "SourceLineGroup": line_group_name(saved_line),
        "fit_status": status,
...
def normalize_fit_row(
    ...
    return {
        ...
        "LSFAlreadyIncluded": True,
        "IndividualLineSpectrumUsed": bool(load_meta.get("individual_line_spectrum_used", True)),
        "SpectrumBranch": finite_or_blank(load_meta.get("spectrum_branch", "")),
        "SourceLineGroup": finite_or_blank(load_meta.get("line_group", line_group_name(saved_line))),
...
def fit_one_spectrum(
    ...
    for iline, saved_line in enumerate(selected_line_labels(args)):
        ...
        try:
            if args.verbose:
                print(
                    ...
                    f"branch={args.spectrum_branch}"
                )

            wave, flux_clean, load_meta = load_individual_line_spectrum_h5(
                spectrum_file=spectrum_file,
                saved_line=saved_line,
                branch=args.spectrum_branch,
            )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        choices=["lsf", "raw"],
        default="lsf",
        help="Per-line spectrum branch to fit. Default lsf matches the existing LSF-included fitter.",
    )
    parser.add_argument(
        "--line-labels",
        default="all",
        help="Comma-separated saved line labels to fit, or all. Useful for smoke tests.",
    )
