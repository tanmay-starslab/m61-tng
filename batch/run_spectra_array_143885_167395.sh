#!/bin/bash
#SBATCH --job-name=m61_spec_rerun_subset
...
#SBATCH --output=/scratch/tsingh65/m61-tng/logs/spec_rerun_subset_%A_%a.out
#SBATCH --error=/scratch/tsingh65/m61-tng/logs/spec_rerun_subset_%A_%a.err
#SBATCH --array=1-2   # task 1: SID 143885, task 2: SID 167395

# Notes:
# - Array maps SLURM_ARRAY_TASK_ID -> SID_LIST line number.
# - Fixed rerun list: SID 143885 (flip only) and SID 167395 (noflip + flip).
# - Everything else stays aligned with the main spectra array job.
...
SID_LIST="${REPO}/data/sids_from_cutouts_snap${SNAP}_rerun_143885_167395.txt"
REBUILD_SID_LIST=1
...
SID="$(sed -n "${TASK_ID}p" "${SID_LIST}" | tr -d '[:space:]')"
if [[ -z "${SID}" || ! "${SID}" =~ ^[0-9]+$ ]]; then
  echo "[WARN] Empty/invalid SID for SLURM_ARRAY_TASK_ID=${SLURM_ARRAY_TASK_ID:-NA}; skipping."
  exit 0
fi
...
case "${SID}" in
  ...
  167395)
    ...
    ;;
  *)
    echo "[ERROR] Unexpected SID=${SID}; expected 143885 or 167395."
    exit 1
    ;;
esac
...
# -----------------------------
# SUBMIT NOTE (manual):
#   sbatch run_spectra_array_143885_167395.sh
# -----------------------------
